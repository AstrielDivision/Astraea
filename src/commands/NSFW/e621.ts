...
@ApplyOptions<NorthCommandOptions>({
	...
	detailedDescription: 'Returns an Image from e621. Use --results=[number] or -r=[number] if you wish to have more than one image',
	cooldownLimit: 2,
	nsfw: true,
	cooldownDelay: 5000,
	options: ['results', 'r']
	...
	public async run (message: Message, args: Args): Promise<Message> {
		...
		for await (let re of req) {
			const embed = new MessageEmbed()
				...
				.setFooter(`Artist(s): ${re.tags.artist.join(', ')} | Ups: ${re.score.up} | Downs ${re.score.down} | Total Score: ${re.score.total}`)
				.setColor('RANDOM')
